...
3. [スプライト表示（スプライトアトリビュート&スプライトカラー）](#スプライト表示)  
    ...
    2. [スプライトモード2](#スプライトモード2)  
        ...
        4. [スプライトモード2 - SCREEN7/8/9/10/11/12 （256ライン）](#スプライトモード2---SCREEN789101112-256ライン)
...
> **Warning**
>
> BASICの```COLOR```命令でパレット操作しているとVRAMのパレットテーブルが邪魔なので、
> ```COLOR=(n,r,g,b)```でのパレット操作は使わない方が良いです。
> 
> ※ ```COLOR=(n,r,g,b)```でのパレット操作は使わない前提のメモになります。
...
各テーブルは、ベースアドレスを設VDPへ定することで、表示に使用するVRAMアドレスを変更できます。

ベースアドレスは画面モードによって、アドレスの設定単位に制限があります。

> **Note**  
>
> 空き領域をフルに使いたい場合はカラーパレットテーブルが邪魔になります。
>
> BASICのCOLOR命令によるカラーパレット変更を行わないようにし、  
> VDPレジスタで直接カラーパレット設定するようにすればVRAMパレットテーブルが不要になります。
...
VRAM 32KB（$0000 ～ $4000）の範囲ではVRAMの空き領域がないので、ダブルバッファを確保することはできません。  
...
縦スクロールで255ライン全てを使う場合、  
ピクセル表示だけで$0000-$7FFFを使ってしまいます。

この場合、ページ0にスプライトテーブルを置くことはできないので、  
ページ1に確保します。
...
16x16の場合パターン64個なので、  
32*2ページのパターンバッファとして扱い、  
スプライト番号につき表示パターンと裏パターンの2個を割り当てる。
という手法を紹介します。
...
|レジスタ|bit7 |bit6 |bit5 |bit4 |bit3 |bit2 |bit1 |bit0 | 計算式           |
|--------|-----|-----|-----|-----|-----|-----|-----|-----|------------------|
|VDP R#2 |  0  | A16 | A15 | A14 | A13 | A12 |  1  |  1  | ```ad / $1000``` |

パターンジェネレータテーブルがあるので $1000 は使えません。

よって、$0000, $2000, $3000 のみです。

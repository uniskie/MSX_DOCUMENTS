...
- 2025/05/03 (2)
  ...
  - 7部-4章-5 Y8950(MSX-AUDIO) の式svgをTeXに置き換えて統一
  - サイドバーに元サイトへのリンク追加
...
既にダウンロード済みのファイルはダウンロードしない為、
再ダウンロードをしたいときは
- ページのhtmlは「org」フォルダ
- 画像は「img」フォルダ
- js等は「theme」フォルダ

をフォルダごと削除するか、更新したいファイルを削除してから、
「`make_offline.ps1`」を実行してください。
...
3. TeX（LaTeX）式の変換
   ...
   1. 7部-4章-5 Y8950(MSX-AUDIO) の式svgをTeXに置き換える （力業）
   2. Google Charts APIを呼び出している箇所をTeX書式に変換
   3. 調整＆記載ミス修正して置き換える
...
既にSVGファイルに変換済みのもTeX式と置き換えてしまった方が表示がキレイになるので、
SVG形式の式画像をTeX式に置換しています。
（7部-4章-5 Y8950(MSX-AUDIO) の式7.8～式7.10）
...
> $replace_svg_to_TeX が $True の場合、  
> 7部-4章-5 Y8950(MSX-AUDIO) の式7.8～7.10を
> svg画像へのリンクからTeX式に置き換えます。  
> （このほうが表示が統一される）

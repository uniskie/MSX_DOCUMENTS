...
- 数式表示はオンライン接続が必要
  これはブラウザセキュリティポリシーの都合で、KATEXのjsファイル、css、fontなどをオンラインから読み込む必要がある為です。

- 一部の数式はエラーが出る
  Google Chart APIへ渡していた式だと、エラーが出る箇所が幾つかあります

他に完全オフラインで使用できるTEX表示jaxascriptライブラリがあればそちらに切り替えたい所です。
...
- `_latex.ps1`  
  ...
  - Google Chart Api廃止で表示されなくなっていたので、LATEX式に置換
...
_latex.ps1 ファイルを弄ると、TEX変換方法を変更できます

githubに置いてあるものは$use_KATEXが有効です。

$use_KATEX も $use_MathJax もどちらも無効の場合は、
TEXレンダリングライブラリを使いません。
TEX式コマンドをそのまま表示します。
